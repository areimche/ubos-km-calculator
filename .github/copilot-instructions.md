...
## What this is
A static web app (`index.html`, `app.js`, `styles.css`) with no frontend framework and no client-side build step. It displays total kilometers between two teams/locations, reading precomputed distance data from `data/kilometertabelle.json` via `fetch()` at page load.
...
## Architecture
- **Distance model** (`app.js`): `kmGraph` is a `Map<location, Map<otherLocation, km>>`, built from the JSON adjacency object at startup (`buildGraphFromData`). Only **direct** lookups between two selected locations are supported — there is no shortest-path/graph traversal between indirectly connected locations.
...
- **State/UI wiring**: No framework — plain DOM manipulation with cached element references at the top of `app.js`. Result messages are tracked via `lastResultMessage` (key + vars) so `renderCurrentResult()` can re-render the last result when the language is switched without recalculating.
